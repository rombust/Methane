...
void CGameTarget::InitGame()
{
	...
	filename = resource_dir + "instructions.png";
	image = clan::ImageProviderFactory::load(filename);
	m_Instructions = clan::Texture2D(m_Canvas, image.get_width(), image.get_height());
	m_Instructions.set_image(m_Canvas, image);
...
//!	\return 
//------------------------------------------------------------------------------
void CGameTarget::DrawInstructions()
{
	clan::Rectf rect(m_Instructions.get_size());
	m_Batcher->draw_image(m_Canvas, rect, rect, 0.0f, m_Instructions, 0.0f);
}
